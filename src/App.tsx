...
import { computeSeoMeta, parseRoute, type RouteState } from "./lib/seoMeta";
...
function useSeo(route: RouteState) {
  const { language, page, toolsPage, guidesPage, hasLanguagePrefix, isFallbackRoute } = route;

  useEffect(() => {
    // computeSeoMeta is the shared source of truth (src/lib/seoMeta.ts); the
    // build-time prerender injects the exact same values into static HTML.
    const meta = computeSeoMeta(route);
    ...
    setJsonLd("instantmail-dynamic-schema", meta.jsonLd);

    clearAlternateLinks();
    meta.alternates.forEach((alternate) => {
      setLink("alternate", alternate.href, alternate.hreflang);
    });
    // route is reconstructed from the same primitive fields each render, so
    // depend on those primitives rather than the object identity.
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [language, page, toolsPage, guidesPage, hasLanguagePrefix, isFallbackRoute]);
}
